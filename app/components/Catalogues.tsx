...
import { PdfIcon, ExternalLinkIcon, FacebookIcon } from './Icons';

type CataloguesProps = {
  facebookUrl: string;
};

export default function Catalogues({ facebookUrl }: CataloguesProps) {
  return (
    <div className="mx-auto mt-16 grid max-w-3xl grid-cols-1 gap-6 sm:grid-cols-2">
      {catalogues.map((catalogue) => {
        ...
        return (
          <div
            key={catalogue.id}
            className="group flex flex-col rounded-xl bg-white p-6 shadow-lg transition-all duration-300 hover:shadow-xl"
          >
      ...
      <div className="group flex flex-col rounded-xl bg-white p-6 shadow-lg transition-all duration-300 hover:shadow-xl">
        <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-primary-100 text-primary-600 transition-colors duration-300 group-hover:bg-primary-600 group-hover:text-white">
          <FacebookIcon className="h-7 w-7" />
        </div>

        <h3 className="mt-6 text-xl font-semibold text-gray-900">
          Volkswagen &amp; More on Facebook
        </h3>
